package com.portfolio.backend.application.views.aboutmeview;

import com.portfolio.backend.application.data.SampleBook;
import com.portfolio.backend.application.services.SampleBookService;
...
@PageTitle("AboutMeView")
@Menu(icon = "line-awesome/svg/info-solid.svg", order = 1)
@Route(value = "AboutMeView/:sampleBookID?/:action?(edit)")
@RolesAllowed("ADMIN")
public class AboutMeViewView extends Div implements BeforeEnterObserver {

    private final String SAMPLEBOOK_ID = "sampleBookID";
    private final String SAMPLEBOOK_EDIT_ROUTE_TEMPLATE = "AboutMeView/%s/edit";
    ...
    public AboutMeViewView(SampleBookService sampleBookService) {
        this.sampleBookService = sampleBookService;
        addClassNames("about-me-view-view");
        ...
        grid.asSingleSelect().addValueChangeListener(event -> {
            if (event.getValue() != null) {
                ...
                clearForm();
                UI.getCurrent().navigate(AboutMeViewView.class);
            }
        });
    ...
    public void beforeEnter(BeforeEnterEvent event) {
        ...
        if (sampleBookId.isPresent()) {
            ...
            if (sampleBookFromBackend.isPresent()) {
                ...
                // refresh grid
                refreshGrid();
                event.forwardTo(AboutMeViewView.class);
            }
        }
    }
